 ...
 * @property {?ShadowRoot} [initShadowRoot] - `options.shadowRoot` when passed (for selection API)
 */
...
function Constructor(editorTargets, options) {
	...
	// set carrier wrapper
	const mountRoot = options?.shadowRoot;
	if (mountRoot) {
		mountRoot.appendChild(editor_carrier_wrapper);
	} else {
	...
	return {
		...
		initShadowRoot: options?.shadowRoot || null,
		options: o,
		plugins: plugins,
		icons: icons,
		lang: lang,
