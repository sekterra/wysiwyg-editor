...
export default class ContextProvider {
	/**
	 * @description `options.shadowRoot` from create(), when set
	 * @type {?ShadowRoot}
	 */
	#initShadowRoot;
	 ...
	 */
	constructor(product) {
		this.#initShadowRoot = product.initShadowRoot || null;
	...
	#setEditorParams(e) {
		...
		if (!frameOptions.get('iframe') && typeof ShadowRoot === 'function') {
			const wwFrame = e.get('wysiwygFrame');
			let detected = null;
			let child = wwFrame;
			while (child) {
				if (child.shadowRoot && child.shadowRoot.contains(wwFrame)) {
					detected = child.shadowRoot;
					break;
				}
				if (child instanceof ShadowRoot && child.contains(wwFrame)) {
					detected = child;
					break;
				}
