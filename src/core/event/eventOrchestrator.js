...
class EventOrchestrator extends KernelInjector {
	...
	_addCommonEvents() {
		...
		/** menu event */
		this.#eventManager.addEvent(this.#context.get('menuTray'), 'mousedown', buttonsHandler, true);
		this.#eventManager.addEvent(this.#context.get('menuTray'), 'click', OnClick_menuTray.bind(this), true);

		/** toolbar event */
		this.#eventManager.addEvent(this.#context.get('toolbar_main'), 'mousedown', buttonsHandler, true);
		this.#eventManager.addEvent(this.#context.get('toolbar_main'), 'click', toolbarHandler, false);
		// subToolbar
		if (this.#options.has('_subMode')) {
			this.#eventManager.addEvent(this.#context.get('toolbar_sub_main'), 'mousedown', buttonsHandler, true);
		...
		if (this.#contextProvider.shadowRoot) {
			this.#shadowToolbarPointerDown = this.#eventManager.addGlobalEvent('pointerdown', this.#onShadowToolbarPointerDownCapture.bind(this), true);
		}
	...
	_removeAllEvents() {
		...
		this.#shadowToolbarPointerDown &&= this.#eventManager.removeGlobalEvent(this.#shadowToolbarPointerDown);
	...
	/**
	 * @internal
	 * @description Shadow: capture expanded selection before toolbar interaction clears `ShadowRoot.getSelection()`.
	 * @param {PointerEvent} ev
	 */
	#onShadowToolbarPointerDownCapture(ev) {
		const t = dom.query.getEventTarget(ev);
		if (!t) return;
		const tb = this.#context.get('toolbar_main');
		const st = this.#options.has('_subMode') ? this.#context.get('toolbar_sub_main') : null;
		if (!(tb && tb.contains(t)) && !(st && st.contains(t))) return;
		if (dom.check.isInputElement(t)) {
			this.$.store.set('_toolbarPreserveSelection', false);
			return;
		}
		if (this.$.selection.snapshotExpandedRangeIfLive()) {
			this.$.store.set('_toolbarPreserveSelection', true);
		} else {
			this.$.store.set('_toolbarPreserveSelection', false);
		}
	}
	...
	#OnSelectionchange_document(_wd) {
		const shadow = this.#contextProvider.shadowRoot;
		const selection =
			shadow && typeof shadow.getSelection === 'function' ? shadow.getSelection() : _wd.getSelection();
