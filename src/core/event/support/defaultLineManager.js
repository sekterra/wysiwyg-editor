	...
	execute(formatName) {
		...
		try {
			if (commonCon.nodeType === 3) {
				const parent = commonCon.parentNode;
				if (!parent) return;

				const tag = String(formatName || this.#$.options.get('defaultLine')).toLowerCase();
				const doc = commonCon.ownerDocument || this.#$.frameContext.get('_wd');
				format = doc.createElement(tag);
				parent.insertBefore(format, commonCon);
				format.appendChild(commonCon);
			}
		} catch {
			const lineTag = formatName || this.#$.options.get('defaultLine');
			const wd = this.#$.frameContext.get('_wd');
			const $ = this.#$;
			queueMicrotask(() => {
				try {
					wd.execCommand('formatBlock', false, `<${lineTag}>`);
				} catch (_) {
					/* formatBlock may fail if selection is gone */
				}
