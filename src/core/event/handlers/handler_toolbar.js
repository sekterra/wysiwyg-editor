...
export function ButtonsHandler(e) {
	...
	// Blur runs before click; it clears table multi-cell state via `offCurrentController` → `resetInfo`.
	// Snapshot cells on toolbar/menu mousedown (capture) so bulk format still sees them on click.
	if (!dom.check.isInputElement(eventTarget) && !this.$.frameContext.get('isCodeView')) {
		const cells = this.$.plugins.table?.state?.selectedCells;
		if (cells?.length >= 2) {
			this.$.store.set('_tableMultiCellFormatSnapshot', cells.slice());
		}
	}
	let toolbarSnapOk = !!this.$.store.get('_toolbarPreserveSelection');
	if (!dom.check.isInputElement(eventTarget)) {
		if (!toolbarSnapOk) {
			toolbarSnapOk = this.$.selection.snapshotExpandedRangeIfLive();
			if (toolbarSnapOk) this.$.store.set('_toolbarPreserveSelection', true);
		}
	} else {
	...
	} else if (!toolbarSnapOk && !this.$.frameContext.get('wysiwyg').contains(this.$.selection.getNode())) {
...
export function OnClick_toolbar(e) {
	const eventTarget = dom.query.getEventTarget(e);
	const r = this.$.store.get('_range');
	if (this.$.store.get('_toolbarPreserveSelection') && r && !r.collapsed) {
		try {
			this.$.frameContext.get('wysiwyg').focus();
			this.$.selection.setRange(r.cloneRange());
		} catch (_) {
			/* ignore */
		}
	}
	...
	this.$.store.set('_toolbarPreserveSelection', false);
	this.$.store.set('_tableMultiCellFormatSnapshot', null);
}
