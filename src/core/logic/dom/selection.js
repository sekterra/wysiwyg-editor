...
class Selection_ {
	...
	/**
	 * @description Live ShadowRoot from context (set after frame init; must not be cached in constructor).
	 * @returns {ShadowRoot|null}
	 */
	#liveShadowRoot() {
		return this.#$.contextProvider.shadowRoot;
	}
	...
	get() {
		...
		const focusInWw = !!(focusNode && wysiwyg.contains(focusNode));

		// Do not call addRange here to "fix" shadow selection: that fires document selectionchange,
		// re-enters init()/toolbar paths, and can trigger Chrome's recursive execCommand warning.
		if (!this.#store.get('_range') && !focusInWw) {
	...
	/**
	 * @description Clone a non-collapsed live selection into the store (used before toolbar `mousedown` clears shadow selection).
	 * @returns {boolean} `true` if an expanded range was stored.
	 */
	snapshotExpandedRangeIfLive() {
		...
		if (!ww.contains(live.startContainer) || !ww.contains(live.endContainer)) return false;
		let clone;
		try {
			clone = live.cloneRange();
		} catch {
			return false;
		}
		this.#store.set('_range', clone);
		this.#rangeInfo(clone, selection);
		return true;
	}
	...
	getRange() {
		...
		const ww = this.#frameContext.get('wysiwyg');
		if (
			this.#store.get('_toolbarPreserveSelection') &&
			range &&
			!range.collapsed &&
			range.startContainer.isConnected &&
			range.endContainer.isConnected &&
			ww.contains(range.startContainer) &&
			ww.contains(range.endContainer)
		) {
			return range;
		}
	...
	init() {
		...
		if (selection.rangeCount > 0) {
			...
			if (this.#store.get('_toolbarPreserveSelection') && range.collapsed) {
				const prev = this.#store.get('_range');
				if (
					prev &&
					!prev.collapsed &&
					ww.contains(prev.startContainer) &&
					ww.contains(prev.endContainer) &&
					prev.startContainer.isConnected &&
					prev.endContainer.isConnected
				) {
					range = prev;
				}
			}
			...
			if (sc && ec && ww.contains(sc) && ww.contains(ec)) {
				try {
					const r = this.#frameContext.get('_wd').createRange();
					r.setStart(sc, selection.anchorOffset);
					r.setEnd(ec, selection.focusOffset);
					range = r;
				} catch {
					range = null;
				}
			...
			if (!range) {
				const prev = this.#store.get('_range');
				const activeElHtml = /** @type {?HTMLElement} */ (activeEl?.nodeType === 1 ? activeEl : null);
				const focusInsideEditorTree =
					!!activeEl &&
					(ww.contains(activeEl) || !!(activeElHtml?.shadowRoot && activeElHtml.shadowRoot.contains(ww)));
				const focusOutsideWw = !!activeEl && !focusInsideEditorTree;
				if (
					prev &&
					!prev.collapsed &&
					ww.contains(prev.startContainer) &&
					ww.contains(prev.endContainer) &&
					focusOutsideWw &&
					!dom.check.isInputElement(activeEl)
				) {
					range = prev;
				} else {
	...
	#rangeInfo(range, selection) {
		...
		if (range.collapsed) {
			...
			selectionNode =
				selection.anchorNode && ww.contains(/** @type {Node} */ (selection.anchorNode)) ? selection.anchorNode : range.startContainer;
		}
